...
2. Publish a GitHub release: `gh release create v0.8.2 --title "v0.8.2" --notes-file <notes>`.
...
2. Build a source archive from the tag, so the upload contains exactly the committed tree:
   ...
   ```bash
   git archive --format=tar.gz --prefix=gdp-wage-transmission-0.8.2/ \
     -o gdp-wage-transmission-0.8.2.tar.gz v0.8.2
   sha256sum gdp-wage-transmission-0.8.2.tar.gz
   ```
...
```bash
tar -xzf gdp-wage-transmission-0.8.2.tar.gz
cd gdp-wage-transmission-0.8.2
